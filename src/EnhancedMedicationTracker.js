...
class EnhancedMedicationTracker {
  ...
  addMedication(medicationData) {
    ...
    if (processedData.dosage && processedData.unit) {
      const dosageStr = `${processedData.dosage}${processedData.unit}`;
    ...
    if (processedData.dosage && processedData.dosage > this.validationRules.dosageValidation.maxValue) {
    ...
    // Validate frequency
    const frequencyValid = this._validateFrequency(processedData.frequency);
    ...
    for (const [id, med] of this.medications.entries()) {
      const medDosageStr = typeof med.dosage === 'number' ? `${med.dosage}${med.unit}` : med.dosage;
      const newDosageStr = typeof processedData.dosage === 'number' ? `${processedData.dosage}${processedData.unit}` : processedData.dosage;
      if (med.name === processedData.name && medDosageStr === newDosageStr) {
    ...
    const medication = {
      ...
      dosage: processedData.dosage,
      unit: processedData.unit,
      frequency: processedData.frequency,
      prescriber: processedData.prescriber || 'Unknown',
      reason: processedData.reason || 'Not specified',
